...
## 1.3. 使用Vivado HLS对FCS-MPC算法进行加速
  ...
  模型预测控制算法的实现主要是在Vivado HLS和Vivado两个EDA设计工具中完成。首先使用Vivado HLS对模型预测控制算法进行加速，该部分完成的是实现C++高级语言到寄存器级硬件描述语言（Verilog）的转化，并将其封装成后续可进行图形化模块设计的IP核；然后在Vivado设计套件中完成基于IP核的模块化设计（Block Design）进而完成寄存器传输级（RTL）到比特流的FPGA设计。基于HLS的模型预测控制算法设计流程如下图所示。
  <center>
...
# 项目总结
  ...
  在冬令营讲座阶段，我系统学习了基于HLS的加速原理、vivado hls的使用以及较为全面的学习了最新的VITIS开发工具。在项目阶段，我参考了HLS高效设计方法指南给出的基本步骤，在对各种directive按步骤进行分类归纳学习的同时，尝试尽可能多的使用各种directive来加速模型预测控制算法，并从实际加速效果上对比分析各个directive的异同，进一步对讲座阶段的知识进行加深巩固。此外在这次冬令营活动中，我还参加了由xilinx的老师们精心组织准备的由各位学者、研究员以及优秀学员主持的包含各种前沿技术的网上研讨会，这些借助xilinx硬件实现的各类先进技术和前沿研究极大的拓宽了我的视野。
  
  最后，感谢陆老师以及xilinx的各位老师在这次冬令营活动中的辛勤付出，同时感谢stefan，谢谢你让我有幸学习了如此优秀的项目，祝各位老师在新的一年里事业有成，一切顺利。